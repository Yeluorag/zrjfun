...
void 
zf_open_string_literal(void)
{
	st_string_literal_buffer_size = 0;
}

void 
zf_add_string_literal(int letter)
{
...
void 
zf_reset_string_literal_buffer(void)
{
...
char *
zf_close_string_literal(void)
{
	char *new_str;

	new_str = zf_malloc(st_string_literal_buffer_size + 1);
...
char *
zf_create_identifier(char *str)
{
	char *new_str;

	new_str = zf_malloc(strlen(str) + 1);
	strcpy(new_str, str);
